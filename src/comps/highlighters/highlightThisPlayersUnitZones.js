...
const highlightThisPlayersUnitZones = (units, zones, thisPlayersRole, addedClassName) => {
      ...
      if (thisPlayersUnits.length) {
        for (let j = 0; j < thisPlayersUnits.length; j += 1) {
          ...
          if (oneAreaZone.toString() === thisUnitsMainZone.toString()) {
            oneAreaElement.classList.add(addedClassName);
          }
        }
      }
    }
...
export default highlightThisPlayersUnitZones;
