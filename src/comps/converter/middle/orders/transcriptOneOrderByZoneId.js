...
const transcriptOneOrderByZoneId = (rs, zoneId, t) => {
  let strOutput = '';
  let arrOutput = [];
  ...
  const { thisOrders, thisVariant } = rs;
  const dataZones = rs[`dataZones|${thisVariant}`];

  if (thisOrders.length && dataZones.length) {
    for (let i = 0; i < thisOrders.length; i += 1) {
      const [unitTypeO, , orderTypeO, activeZoneO,
        passiveZoneO, targetZoneO] = thisOrders[i];
...
export default transcriptOneOrderByZoneId;
